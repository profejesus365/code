...
- **Portada** — da la bienvenida, cuenta la historia de la Academia (la IA *GHOST* ha
  hackeado el sistema de la Escuela) y ofrece cuatro caminos: Hacker Tutor, Hacker
  Estudiante, Recursos didácticos y Exámenes.
- **Panel del estudiante** — entra con su código y ve su progreso convertido en juego:
  XP, los 10 niveles hacker, insignias, ranking, boletines y escarapela digital.
- **Panel del docente** — entra con su código de acceso y gestiona todo: estudiantes,
  notas por misión y periodo, estadísticas, ranking, escarapelas, informes, datos de la
  institución y copias de seguridad.
...
historia de la Academia y aparece la columna con los cuatro caminos:

| Camino | A dónde lleva |
|---|---|
| 🟡 **Hacker Tutor** | Pide el código del docente y abre el panel de gestión |
| 🔵 **Hacker Estudiante** | Pide el código de estudiante y abre su panel personal |
| 🟢 **Recursos didácticos** | Catálogo de 23 plataformas educativas con filtros |
| 🟣 **Exámenes** | Portal de exámenes de la institución (enlace configurable) |

La misma pantalla lleva al pie el bloque de contacto para docentes interesados en
recursos de gamificación y aplicaciones de registro.

> El enlace del portal de exámenes se pone en **Consola ▸ Datos de la institución ▸
> Portal de exámenes**. Mientras esté vacío, el botón avisa de que aún no está configurado.

---

## 🔑 Cómo se entra

Cada camino pide su propio código y **no acepta el del otro rol**:

| Quién | Qué escribe | Qué ve |
|---|---|---|
| Estudiante | Su código (`CODE-0001`, `CODE-0002`, …) | Su panel personal, de solo lectura |
| Docente | **`profejesus365`** (cambiable) | El panel de gestión completo |

- La casilla **«Mantener la sesión abierta»** recuerda el código en ese navegador y la
  próxima vez entra directo al panel, saltándose la portada.
- **Salir** borra la sesión y devuelve a la pantalla de la misión.
- El código del docente se cambia en **Consola ▸ Código de acceso**. Pide el actual, el
  nuevo (mínimo 6 caracteres) y su confirmación.

> 🔒 **Sobre la seguridad.** El código docente separa un panel del otro y evita que un
> estudiante entre por curiosidad, pero **no es una contraseña segura**: toda la aplicación
> se ejecuta en el navegador y el archivo de datos viaja con el proyecto. Guárdalo como
> guardarías la llave del salón. En el archivo se almacena transformado, no en texto plano,
> para que no se lea de un vistazo al abrir el JSON.
...
| **Escarapelas** | Las credenciales de todo el curso, siempre al día con lo último que registraste. Toca una para verla en grande. Se descargan en **PNG, PDF e HTML interactivo**, sueltas o en lote |
...
| **Consola** | Datos de la institución, ponderaciones, código de acceso, carpeta de datos, respaldos, diagnóstico y vaciado de la base |

**Atajos:** `Ctrl + 1 … 7` cambian de sección · `Esc` cierra ventanas · `Ctrl + P` imprime.
...
Un catálogo de **23 plataformas educativas**, tomado de la compilación de
[Exploradores del Saber](https://santarita.exploradoresdelsaber.com/recursos-educativos-digitales/),
agrupadas en cinco categorías: evaluación y gamificación, creación de contenidos,
lógica y programación, práctica y matemáticas, y lectura y cultura.

Se filtran por **categoría** y por **búsqueda** (que mira el nombre, la descripción, la
categoría y las áreas a la vez). El contador muestra en todo momento cuántos recursos
quedan y de qué categorías son.

Todos los enlaces abren en una pestaña nueva con `rel="noopener noreferrer"`.
Para añadir, quitar o reetiquetar recursos, edita el arreglo `RECURSOS` de
`js/recursos.js`.
...
Doble clic en **`actualizar-web.bat`** (o `python publicar.py`). Hace tres cosas:

1. Congela la base actual en `data/base.js`, como una etiqueta `<script>`.
2. Junta los 28 módulos de `js/` en un solo `js/app.bundle.js` clásico, sin `import`.
3. Deja la carpeta **`publicar/`** lista.

Con eso, la carpeta `publicar/` se puede:
...
Cada vez que cambies notas, vuelve a ejecutar `actualizar-web.bat` y vuelve a subir.

> En la versión publicada el panel del docente sigue funcionando, pero guarda solo en el
> navegador de quien lo abra. La base de verdad es la del proyecto original en tu equipo.
...
├── publicar/                 Versión lista para subir (la genera actualizar-web)
...
│   ├── img/favicon.svg
│   └── vendor/               Chart.js, html2canvas, qrcodejs y SheetJS locales
└── js/
    ...
    ├── tarjeta.js            Escarapela y boletín compartidos por los dos paneles
    ├── ui.js                 DOM, toasts, modales, animaciones
    ├── graficas.js           Capa sobre Chart.js
    ├── bus.js                Aviso central de «los datos cambiaron»
    ├── exportar.js           PNG, PDF carta y HTML interactivo
    ├── portada.js            Bienvenida, narrativa, accesos y contacto
    ├── recursos.js           Catálogo de recursos educativos con filtros
    ├── app.js                Arranque, acceso por código y enrutador de los dos paneles
    ├── vistas/               Las 7 vistas del estudiante
    │   ├── perfil.js · niveles.js · estadisticas.js · comparativa.js
    │   └── ranking.js · boletines.js · escarapela.js
    └── docente/              Las 7 vistas del docente
        ├── filtros.js            Desplegables hechos con lo que hay en la base
        ├── centro.js · estudiantes.js · notas.js · clasificacion.js
        └── credenciales.js · informes.js · consola.js
```
...
Las cinco librerías críticas se piden primero al CDN y, **si no hay internet, caen solas a
...
| Código de acceso del docente | Panel docente ▸ **Consola ▸ Código de acceso** |
| Código de fábrica (antes del primer cambio) | `CODIGO_DOCENTE_INICIAL` en `js/config.js` |
...
- El panel del docente sí ve y edita todo, y está detrás del código de acceso.
